...
public class BMIView extends JFrame {
	...
	class PanelBelow extends JPanel{
		public PanelBelow() {
			...
			ImageIcon icon = new ImageIcon("image/bmi_originals.jpg");
	...
	ActionListener btnL = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(vo.getBmi() < 18.5) {
				bmiResult = "저";
				icon = new ImageIcon("image/under.jpg");
			}else if(vo.getBmi() < 24.9) {
				bmiResult = "정상";
				icon = new ImageIcon("image/normal.jpg");
			}else if(vo.getBmi() < 29.9) {
				bmiResult = "과";
				icon = new ImageIcon("image/over.jpg");
			}else if(vo.getBmi() < 34.9) {
				bmiResult = "비만";
				icon = new ImageIcon("image/obese.jpg");
			}else {
				bmiResult = "고도비만";
				icon = new ImageIcon("image/extremely.jpg");
			}
